...
public class EditarClienteController implements Initializable {
	...
	public String validarDatos() {
		...
		matcherNumeroCliente = patternNumeroCliente.matcher(txtNumeroCliente.getText());
		if (!matcherNumeroCliente.matches()) {
			txtNumeroCliente.setStyle("-fx-text-fill: red;");
			ivErrorNumeroCliente.setImage(imageError);
	...
	public void buscar() {
		...
		if (txtNumeroCliente.getText().isEmpty()) {
			...
			cuadroDialogo.setContentText("El numero de cliente esta vacio.");
		...
		}else if (!matcherNumeroCliente.matches()) {
			txtNumeroCliente.setStyle("-fx-text-fill: red;");
			ivErrorNumeroCliente.setImage(imageError);
			Alert cuadroDialogo = new Alert(AlertType.ERROR);
			cuadroDialogo.setContentText("El numero de cliente debe tener 12 caracteres numericos.");
			cuadroDialogo.setTitle("Error");
			cuadroDialogo.setHeaderText("Error ");
			cuadroDialogo.showAndWait();
			btnActualizar.setDisable(true);
			...
			Cliente cliente = conexion.buscarEditarCliente(txtNumeroCliente.getText());
			if (cliente==null){
				txtNumeroCliente.setStyle("-fx-text-fill: red;");
				ivErrorNumeroCliente.setImage(imageError);
				Alert cuadroDialogo = new Alert(AlertType.ERROR);
				cuadroDialogo.setContentText("El numero de cliente No existe");
				cuadroDialogo.setTitle("Error");
				cuadroDialogo.setHeaderText("Error ");
				cuadroDialogo.showAndWait();
				btnActualizar.setDisable(true);
				...
				txtNumeroIdentidad.setText(cliente.getNumeroIdentidad());
				txtNombres.setText(cliente.getNombres());
				txtApellidos.setText(cliente.getApellidos());
				txtNumeroTelefono.setText(cliente.getTelefono());
				txtNumeroCelular.setText(cliente.getCelular());
				...
				dprFechaNacimiento.setValue(LocalDate.of(calendar.get(calendar.YEAR), (calendar.get(calendar.MONTH) + 1), calendar.get(calendar.DAY_OF_MONTH)));
				cboEstadoCivil.getSelectionModel().select(cliente.getEstadoCivil()-1);
				Direccion direccion= conexion.buscarDireccion(cliente.getCodigoDireccion());
				txtDireccion.setText(direccion.getDescripcionDireccion());
				int codigoMunicipio =conexion.buscarMunicipio(direccion.getCodigoColonia());
				int codigoEstado= conexion.buscarDepartamento(codigoMunicipio);
				cboEstado.getSelectionModel().select(codigoEstado-1);
				cboMunicipio.getSelectionModel().select(codigoMunicipio -1);
				cboColonia.getSelectionModel().select(direccion.getCodigoColonia()-1);
				Empresa empresa = conexion.buscarEmpresa(cliente.getCodigoEmpresa());
				Direccion direccionEmpresa = conexion.buscarDireccion(empresa.getCodigoDireccion());
				txtDireccionEmpresa.setText(direccionEmpresa.getDescripcionDireccion());
				int codigoMunicipioEmpresa =conexion.buscarMunicipio(direccionEmpresa.getCodigoColonia());
				int codigoEstadoEmpresa= conexion.buscarDepartamento(codigoMunicipioEmpresa);
				cboEstadoEmpresa.getSelectionModel().select(codigoEstadoEmpresa-1);
				cboMunicipioEmpresa.getSelectionModel().select(codigoMunicipioEmpresa -1);
				cboColoniaEmpresa.getSelectionModel().select(direccionEmpresa.getCodigoColonia()-1);
				txtNombreEmpresa.setText(empresa.getNombreEmpresa());
				txtTelefonoEmpresa.setText(empresa.getTelefono());
				int tipoTrabajo= conexion.buscarTipoTrabajo(cliente.getCodigoCargo());
				cboTipoTrabajo.getSelectionModel().select(tipoTrabajo-1);
				cboCargo.getSelectionModel().select(cliente.getCodigoCargo()-1);
				cboProfesion.getSelectionModel().select(cliente.getCodigoProfesion()-1);
				txtSalario.setText(""+cliente.getSalario());
	...
	public void editar() {
		...
		if (txtNumeroCliente.getText().isEmpty()) {
			...
			Cliente cliente = new Cliente(txtNumeroCliente.getText(), cboEstadoCivil.getSelectionModel().getSelectedItem().getCodigoEstadoCivil(),
			...
			String mensajeCliente=conexion.actualizarCliente(cliente);
			Alert cuadroDialogo = new Alert(AlertType.INFORMATION);
			cuadroDialogo.setContentText(mensajeCliente + ".");
	...
	public void eliminar(){
		...
		if (txtNumeroCliente.getText().isEmpty()) {
			...
			conexion = new Conexion();
			String mensaje = conexion.eliminarCliente(txtNumeroCliente.getText());
			Alert cuadroDialogo = new Alert(AlertType.INFORMATION);
