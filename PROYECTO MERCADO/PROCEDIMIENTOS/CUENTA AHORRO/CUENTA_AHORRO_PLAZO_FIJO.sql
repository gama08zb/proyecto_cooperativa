create or replace PROCEDURE SP_CREAR_CUENTA_AHORRO_PLAZO(
  ...
  BEGIN
    ...
    IF (V_CANTIDAD_CLIENTE =1) THEN
        IF (V_ESTADO_CLIENTE =1) THEN
            IF (P_FECHA_FINALIZACION>=SYSDATE) THEN 
                SELECT COUNT(*) INTO V_CANTIDAD_CUENTA FROM TBL_CLIENTES_X_CUENTAS_AHORRO A
                INNER JOIN TBL_CUENTAS_AHORRO B
                  ON(A.ID_CUENTA_AHORRO=B.ID_CUENTA_AHORRO)
                WHERE (B.ID_TIPO_CUENTA= P_ID_TIPO_CUENTA)AND (A.ID_CLIENTE=P_ID_CLIENTE) AND ID_ESTADO_CUENTA=1;
                ...
                ELSE
                  P_MENSAJE:= 'ERROR: YA EXITE ESTE TIPO DE CUENTA';
                END IF;
              ELSE
                P_MENSAJE:= 'ERROR: LA FECHA DE FINALIZACION DEBE SER MAYOR';
            END IF;
        ELSE 
